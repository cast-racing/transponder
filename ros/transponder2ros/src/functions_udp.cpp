...
void transponder2ros::init_udp()
{
    ...
    // Get the param
    this->declare_parameter("car_ip", "127.0.0.1");
    this->declare_parameter<int>("transponder.udp_port_read", 15783);
    this->declare_parameter<int>("transponder.udp_port_send", 15783);
    this->declare_parameter<double>("transponder.timeout", 10.0);       // Timeout for alerting user of no packets
    this->declare_parameter<double>("transponder.max_age",  1.0);       // Max age of accepted packets
    ...
    t_Udp_maxAge_ = this->get_parameter("transponder.max_age").as_double();
    ...
    RCLCPP_INFO(this->get_logger(), "\tListening to transponders on port:%d", param_portRead);
